...
//!
//! ```text
//! Authorization: AWS4-HMAC-SHA256 Credential=AKID/20260504/testregion/s3/aws4_request,
...
mod tests {
    ...
    const VALID: &str = "AWS4-HMAC-SHA256 \
        Credential=MockAccessKey/20260506/testregion/s3/aws4_request, \
    ...
    fn test_parse_valid_authorization_header() {
        ...
        assert_eq!(h.access_key, "MockAccessKey");
        ...
        assert_eq!(h.credential_scope, "20260506/testregion/s3/aws4_request");
    }
    ...
    fn test_missing_signature_field_returns_invalid_request() {
        let v = "AWS4-HMAC-SHA256 Credential=AK/20260506/testregion/s3/aws4_request, \
                 SignedHeaders=host";
        let err = SigV4Header::from_authorization_header(v).unwrap_err();
        assert!(matches!(err, FerroxError::InvalidRequest(_)));
    }
    ...
    fn test_malformed_credential_scope_returns_invalid_request() {
        let v = "AWS4-HMAC-SHA256 Credential=AK/20260506/testregion, \
    ...
    fn test_credential_scope_wrong_terminator_returns_invalid_request() {
        let v = "AWS4-HMAC-SHA256 Credential=AK/20260506/testregion/s3/wrong, \
                 SignedHeaders=host, Signature=abc";
        let err = SigV4Header::from_authorization_header(v).unwrap_err();
        assert!(matches!(err, FerroxError::InvalidRequest(_)));
    }
